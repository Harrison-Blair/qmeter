...
func TestFitRows(t *testing.T) {
	...
	result := usage.Result{Windows: []provider.Window{
		{Provider: "claude", Name: "due", RemainingPercent: 10, ResetsAt: now.Add(-time.Hour)},
		{Provider: "codex", Name: "日本語の very long window", RemainingPercent: 80, ResetsAt: now.Add(3*24*time.Hour + 12*time.Hour), RateLimited: true},
		{Provider: "cursor", Name: "exact", RemainingPercent: 30, ResetsAt: now.Add(7 * 24 * time.Hour)},
		{Provider: "opencode-go", Name: "beyond", RemainingPercent: 40, ResetsAt: now.Add(8 * 24 * time.Hour)},
		{Provider: "claude", Name: "unknown", RemainingPercent: 50},
	}, Errors: []usage.ProviderError{{Provider: "failed", Message: "nope"}}}
	before := append([]provider.Window(nil), result.Windows...)

	for _, width := range []int{70, 90, 240} {
		got := FitRows(result, now, width, r, display.Default())
		if !reflect.DeepEqual(result.Windows, before) {
			t.Fatal("FitRows mutated its input")
		}
		if width == MinWidth {
			want := Rows(result, now, width, r, display.Default())
			if !reflect.DeepEqual(got, want) {
				t.Fatalf("minimum fitted rows = %q, want legacy rows %q", got, want)
			}
		}

		nameWidth := min(max(10, runewidth.StringWidth("日本語の very long window"+rateLimitSuffix)), width-60)
		axisWidth := width - nameWidth - 17
		if runewidth.StringWidth(got[0]) != nameWidth+10+axisWidth {
			t.Fatalf("width %d ruler width = %d", width, runewidth.StringWidth(got[0]))
		}
		if !strings.HasPrefix(got[0], strings.Repeat(" ", nameWidth+10)+"now") || !strings.HasSuffix(got[0], "+7d") {
			t.Fatalf("width %d ruler = %q", width, got[0])
		}
		if strings.Index(got[1], "due") < 0 || strings.Index(got[2], "日本") < 0 || strings.Index(got[5], "unknown") < 0 {
			t.Fatalf("width %d rows are not sorted: %q", width, got)
		}
		if !strings.Contains(got[2], rateLimitSuffix) {
			t.Fatalf("width %d rate-limited label = %q", width, got[2])
		}
		for i, marker := range []struct {
			row    int
			marker string
			cell   int
		}{
			...
			axis := fitAxis(t, got[marker.row], nameWidth, axisWidth)
			if axis != strings.Repeat("·", marker.cell)+marker.marker+strings.Repeat(" ", axisWidth-1-marker.cell) {
				t.Errorf("width %d marker %d axis = %q", width, i, axis)
			}
		}
		if axis := fitAxis(t, got[5], nameWidth, axisWidth); strings.TrimSpace(axis) != "" || strings.TrimSpace(string([]rune(got[5])[len([]rune(got[5]))-6:])) != "-" {
			t.Fatalf("width %d unknown row = %q", width, got[5])
		}
	}
}

func TestFitRowsANSIWidthAndRulerAlignment(t *testing.T) {
	r := lipgloss.NewRenderer(io.Discard)
	r.SetColorProfile(termenv.ANSI)
	width := 121
	result := usage.Result{Windows: []provider.Window{{
		Provider: "codex", Name: "日本語", RemainingPercent: 80, ResetsAt: now.Add(time.Hour), RateLimited: true,
	}}}
	rows := FitRows(result, now, width, r, display.Default())
	nameWidth := 10
	axisWidth := width - nameWidth - 17
	if got, want := rows[0], strings.Repeat(" ", nameWidth+10)+"now"+strings.Repeat(" ", axisWidth-6)+"+7d"; got != want {
		t.Fatalf("ruler = %q, want %q", got, want)
	}
	if got := lipgloss.Width(rows[1]); got != width {
		t.Fatalf("ANSI row display width = %d, want %d: %q", got, width, rows[1])
	}
}

func TestFitRowsNarrowUsesExistingTableAndMessages(t *testing.T) {
	r := lipgloss.NewRenderer(io.Discard)
	r.SetColorProfile(termenv.Ascii)
	result := usage.Result{Errors: []usage.ProviderError{{Provider: "failed", Message: "nope"}}, Undetected: []usage.ProviderError{{Provider: "none", Message: "missing"}}}
	if got, want := FitRows(result, now, MinWidth-1, r, display.Default()), Rows(result, now, MinWidth-1, r, display.Default()); !reflect.DeepEqual(got, want) {
		t.Fatalf("narrow FitRows = %q, want %q", got, want)
	}
}

func fitAxis(t *testing.T, row string, nameWidth, axisWidth int) string {
	t.Helper()
	prefix := runewidth.Truncate(row, nameWidth+10, "")
	tail := strings.TrimPrefix(row, prefix)
	axis := string([]rune(tail)[:axisWidth])
	if runewidth.StringWidth(row) != nameWidth+17+axisWidth {
		t.Fatalf("row width = %d, want %d: %q", runewidth.StringWidth(row), nameWidth+17+axisWidth, row)
	}
	return axis
}
