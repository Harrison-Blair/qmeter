...
func TestFilteringAndJSON(t *testing.T) {
	...
	root.SetArgs([]string{"resets", "--filter", "codex, claude ", "--filter", "claude,cursor,opencode-go", "--json"})
	...
	s := out.String()
	if strings.Count(s, `"provider":"claude"`) != 1 || strings.Index(s, `"provider":"claude"`) > strings.Index(s, `"provider":"codex"`) || strings.Contains(s, "secret missing reason") || !strings.Contains(s, `"resets_in_seconds":3000`) || !strings.Contains(s, `"message":"bad request"`) {
		t.Fatal(s)
	}
}
...
func TestTextAndEmptyAndFilter(t *testing.T) {
	for _, filter := range []string{"claude", "cursor", " , "} {
		t.Run(filter, func(t *testing.T) {
			...
			root.SetArgs([]string{"resets", "--filter", filter})
			if err := root.Execute(); err != nil {
				t.Fatal(err)
			}
			if *calls != 1 {
			...
			} else if !strings.Contains(out.String(), "RESETS") {
				t.Fatal(out.String())
			}
		})
	}
...
func TestInvalidInputs(t *testing.T) {
	for _, args := range [][]string{{"resets", "--filter", "nope"}, {"resets", "argument"}, {"resets", "--bogus"}} {
...
func TestCancellation(t *testing.T) {
	for _, args := range [][]string{{"resets"}, {"resets", "--json"}} {
		t.Run(strings.Join(args, " "), func(t *testing.T) {
